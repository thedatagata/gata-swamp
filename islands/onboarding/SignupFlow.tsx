...
export default function SignupFlow({ onComplete }: SignupFlowProps) {
  const [step, setStep] = useState<'plan' | 'addons' | 'account' | 'checkout'>('plan');
  ...
  const handlePlanSelect = (plan: 'starter' | 'smarter') => {
    ...
    // Reset addons when switching plans to avoid incompatible states if any
    ...
    try {
      ...
      const res = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({
          ...
          plan_tier: selectedPlan === 'smarter' ? 'premium' : 'free',
          ...
          <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="bg-[#172217]/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 border-2 border-[#90C137] hover:border-[#a0d147] transition-all">
              ...
              <button 
                ...
                class="w-full py-3 rounded-lg font-bold bg-[#90C137] text-[#172217] hover:bg-[#a0d147]"
              >
                Select Smarter
              </button>
            </div>
    ...
    return (
      <div class="min-h-screen bg-gradient-to-br from-[#172217] to-[#186018] flex items-center justify-center p-4">
        <div class="max-w-2xl w-full bg-[#172217]/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-[#90C137]/30">
          <h2 class="text-3xl font-bold text-[#F8F6F0] mb-6 text-center">Customize Your Plan</h2>
          ...
          <div class="flex gap-4">
            ...
            <button type="button" onClick={handleAddonsSubmit} class="flex-1 py-3 bg-[#90C137] text-[#172217] rounded-lg font-bold hover:bg-[#a0d147]">
              Continue to Account
            </button>
          </div>
        </div>
    ...
    return (
      <div class="min-h-screen bg-gradient-to-br from-[#172217] to-[#186018] flex items-center justify-center p-4">
        <div class="max-w-md w-full bg-[#172217]/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-[#90C137]/30">
          <h2 class="text-3xl font-bold text-[#F8F6F0] mb-6 text-center">Create Account</h2>
          ...
          <form onSubmit={handleAccountSubmit} class="space-y-4">
            ...
            <div class="pt-4 flex gap-4">
              <button type="button" onClick={() => setStep('addons')} class="flex-1 py-3 border border-[#90C137]/50 text-[#F8F6F0] rounded-lg hover:bg-[#90C137]/10">
                Back
              </button>
    ...
    return (
      <div class="min-h-screen bg-gradient-to-br from-[#172217] to-[#186018] flex items-center justify-center p-4">
        <div class="max-w-md w-full bg-[#172217]/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-[#90C137]/30">
          ...
          <div class="flex gap-4">
            <button type="button" onClick={() => setStep('account')} disabled={loading} class="flex-1 py-3 border border-[#90C137]/50 text-[#F8F6F0] rounded-lg hover:bg-[#90C137]/10 disabled:opacity-50">
              Back
            </button>
